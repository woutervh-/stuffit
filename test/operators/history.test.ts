...
import { HistoryOperator } from '../../src/operators/history';
...
describe('HistoryOperator', () => {
    describe('#state', () => {
        ...
        it('Will lazily update its state, ignoring updates from the source if the state is not requested.', () => {
            const store = new HistoryOperator(source, 5);

            source.setState(1);
            source.setState(2);
            chai.assert.deepStrictEqual(store.state, [undefined, undefined, undefined, undefined, 2]);
            chai.assert.deepStrictEqual(store.state, [undefined, undefined, undefined, undefined, 2]);
        });
    });
    ...
    describe('#subscribe', () => {
        ...
        it('Will lazily update its state, even when subscribed to.', () => {
            const store = new HistoryOperator(source, 5);
